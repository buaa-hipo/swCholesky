...
void init_dgemm(double* a, double* b, double* c, double* c2, int lda, int ldb, int ldc, int k, int m, int n){
    int i;
...
int main(int argc, char** argv){
    ...
    char TRANSA, TRANSB;
    double one = 1., zero = 0.;
    init_dgemm(input_a, input_b, input_c, input_c2, lda, ldb, ldc, k, m, n);
int ii;
for (ii = 0; ii < 10; ii++){
    TRANSA = 'N';
    TRANSB = 'C';
    ...
    //athread_join();
    dgemm_single(m, n, k, one, input_a, lda, input_b, ldb, zero, input_c2, ldc);
    gettimeofday(&t2, NULL);
    myTime += TIME(t1, t2);
}
