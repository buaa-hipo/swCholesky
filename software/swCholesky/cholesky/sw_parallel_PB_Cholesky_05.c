...
inline void calcSYRK(double *flops, double *bytes, const int n, const int k, const double alpha, const double beta){
    assert(alpha == 1.0);
    double local_flops = 2.0 * ((1+n)*n*0.5) * k;
    double local_bytes = n*k * 8;
    if (beta != 0.)
      local_bytes += n*n*8;
...
inline void calcGEMM(double *flops, double *bytes, const int m, const int n, const int k, const double alpha, const double beta){
    assert(alpha == 1.0);
    double local_flops = 2.0 * m * n * k;
    double local_bytes = (m*n + m*k + n*k) * 8;
    if (beta != 0.)
      local_bytes += m*n * 8;
...
inline void calcPOTRF(double *flops, double *bytes, const int n){
    double local_flops = n-1 + 1; //column 1, n-1 multi, 1 square-root
    local_flops += 1.0*n*n*n/6.0 + 0.5*n*n - 5.0/3.0*n + 1;
    double local_bytes = ((n+1)*n*0.5) * 2 * 8;

    *flops += local_flops;
    *bytes += local_bytes;
}
inline void calcTRSM(double *flops, double *bytes, const int m, const int n, const double alpha){
    assert(alpha == 1.0);
    double local_flops = n*n*m;
    double local_bytes = ((n+1)*n*0.5 + m*n*2) * 8;

    *flops += local_flops;
    *bytes += local_bytes;
}

inline void calcCOPYIN(double *bytes, const int colNo, const int rowNo){
    assert(rowNo >= colNo);
    double local_bytes = ( (colNo+1)*colNo*0.5 + (rowNo - colNo)*colNo ) * 8;

    *bytes += local_bytes;
}
inline void calcCOPYOUT(double *bytes, const int n1, const int n3){
    double local_bytes = ( (n1+1)*n1*0.5 + n1*n3 ) * 8;
   ...
   for (j1 = levelPtr[i1]; j1 < levelPtr[i1 + 1]; ++j1) {
    ...
    for (k1 = parPtr[j1]; k1 < parPtr[j1 + 1]; ++k1) {
     ...
     if (i == prunePtr[s] - 1) isLast = 1;
      ...
      calcSYRK(&flops, &bytes, ndrow1, supWdts, one, zero);
        #endif
      ...
      PRINT("inner dgemm_ end\n");
       ...
       blasTime += elapsed_seconds;
      calcGEMM(&flops, &bytes, ndrow3,ndrow1,supWdts,one, zero);
        #endif
      }// end of ndrow3
      ...
      #ifdef BLASTIMING
      calcCOPYOUT(&bytes, ndrow1, ndrow3);
      #endif
    ...
    PRINT("inner queue : 1, %d\n", i);
     ...
     blasTime += elapsed_seconds;
     calcTRSM(&flops, &bytes, rowNo, supWdt,one);
#endif
...
printf("/*********END OF ALL FOR**********/\n");
 ...
 printf("@@START ITERATION :: i1, last Level\n");
  ...
  printf("###TOTAL TIME INSIDE numeric is %f\n\n", duration2);
    ...
    printf("@COLLECT::flops is %lf\n", flops);
    printf("@COLLECT::bytes is %lf\n", bytes);
#endif
